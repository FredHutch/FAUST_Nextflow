nf_step_02_make_restriction_matrices <- function(
                                                 gatingSetPath,
                                                 channelBoundsPath
                                                 )
{
    gs <- load_gs(gatingSetPath)
    samplesInExp = flowWorkspace::sampleNames(gs)
    ...
    channelBounds <- readRDS(channelBoundsPath)
    ...
    if (!channelBounds) {
        ...
        for (sampleName in samplesInExp) {
            ...
            exprsMat <- readRDS(paste0(sampleName,"_exprsMat.rds"))#nolint
            if (firstSampleForCB) {
    ...
    saveRDS(channelBoundsUsedByFAUST,paste0("channelBoundsUsedByFAUST.rds"))
    for (sampleName in samplesInExp) {
        exprsMat <- readRDS(paste0(sampleName,"_exprsMat.rds"))#nolint
        ...
        saveRDS(resMat,paste0(sampleName,"_resMat.rds"))
    }
    return()
}
