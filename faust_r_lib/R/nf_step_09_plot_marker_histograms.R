nf_step_09_plot_marker_histograms <- function(
                                              startingCellPop
                                              )
{
    ...
    for (aLevel in uniqueLevels) {
        levelExprs <- readRDS(paste0(aLevel,"_levelExprs.rds"))
    ...
    for (channel in selC) {
        ...
        for (gateNum in seq(totalGateNum)) {
            ...
            p <- .nfGetHistogram(histData,channel,gatesForPlotting)
            cowplot::save_plot(paste0("hist_",channel,"_ab_",gateNum,".pdf"),
                               p,
                               base_height = 7,
                               base_width = 7)
        }
