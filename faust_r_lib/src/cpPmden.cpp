 ...
 * However, these values appear to be coerced to double when the library is
 * loaded for use in an R sesssion. To duplicate library performance, we use the
 * truncated values, and simply store as double here.
 */
std::vector<double> kuip01 = {0.30750198, 0.22031415, 0.15749822, 0.10121863,
                              0.07178070, 0.05080985, 0.03215992};
std::vector<double> kuip02 = {0.26097572, 0.18867970, 0.13705738, 0.08810506,
                              0.06335109, 0.04492857, 0.02858946};
std::vector<double> kuip03 = {0.14897866, 0.11094044, 0.08092217, 0.05257830,
                              0.03748763, 0.02696284, 0.01707126};
std::vector<double> kuip04 = {0.12438262, 0.09529240, 0.06950244, 0.04524029,
                              0.03286277, 0.02348616, 0.01502098};
std::vector<double> kuip05 = {0.09319795, 0.07219891, 0.05339150, 0.03538433,
                              0.02598238, 0.01851747, 0.01174251};
std::vector<double> kuip06 = {0.08018187, 0.06280868, 0.04765458, 0.03164710,
                              0.02301769, 0.01648256, 0.01055136};
std::vector<double> kuip07 = {0.066165059, 0.052643314, 0.040499260,
                              0.027003915, 0.019671301, 0.014140314,
                              0.009111927};
std::vector<double> kuip08 = {0.05768921, 0.04729112, 0.03630651, 0.02443004,
                              0.01784079, 0.01289449, 0.00831476};
std::vector<double> kuip09 = {0.049727442, 0.041269518, 0.031941519,
                              0.021631778, 0.015981589, 0.011540437,
                              0.007469195};
std::vector<double> kuip10 = {0.044093021, 0.037163268, 0.029176417,
                              0.019971113, 0.014785146, 0.010818339,
                              0.006926491};
std::vector<double> kuip11 = {0.038637596, 0.033532556, 0.026499289,
                              0.018379105, 0.013643814, 0.009856368,
                              0.006386310};
std::vector<double> kuip12 = {0.035037285, 0.030666602, 0.024677670,
                              0.017132430, 0.012725232, 0.009228679,
                              0.005995483};
std::vector<double> kuip13 = {0.031642378, 0.028084749, 0.022794216,
                              0.015823040, 0.011929053, 0.008645996,
                              0.005649372};
std::vector<double> kuip14 = {0.029033131, 0.025996755, 0.021296584,
                              0.014971161, 0.011244505, 0.008220232,
                              0.005350641};
std::vector<double> kuip15 = {0.025883091, 0.024057281, 0.020032581,
                              0.014186956, 0.010545471, 0.007767515,
                              0.005061438};
std::vector<double> kuip16 = {0.023257645, 0.022536446, 0.018824915,
                              0.013486222, 0.010061760, 0.007427072,
                              0.004817209};
std::vector<double> kuip17 = {0.020675023, 0.020836156, 0.017644365,
                              0.012805928, 0.009588957, 0.007102941,
                              0.004595116};
std::vector<double> kuip18 = {0.019136349, 0.019431774, 0.016733216,
                              0.012146658, 0.009200609, 0.006771573,
                              0.004431340};
std::vector<double> kuip19 = {0.017891379, 0.018348230, 0.015944010,
                              0.011620645, 0.008794318, 0.006483762,
                              0.004265070};
 ...
 */
std::vector<std::vector<double>> kuipdiffbounds = {
    kuip01, kuip02, kuip03, kuip04, kuip05, kuip06, kuip07,
    kuip08, kuip09, kuip10, kuip11, kuip12, kuip13, kuip14,
    kuip15, kuip16, kuip17, kuip18, kuip19};

std::vector<double> kuipdiffbounds_x = {50.0,   100.0,  200.0, 500.0,
                                        1000.0, 2000.0, 5000.0};

stringInfo cpPmden(const std::vector<double> &xIn) {
  // sorting is current turned off because cpPmden can only be called by
  // tsGates, which is responsible for the sorting. in the event cpPmden is to be
  // used on a stand-alone basis, re-implement the sorting.
  // std::sort(std::begin(x), std::end(x)); //begin by sorting the data vector
  // in place.
  ...
  std::vector<double> currbounds(maxkuipnr,
                                 0.0); // in this port, set maxkuipnr to 19. Do
                                       // not allow user choice of mod.
  ...
  if (nsamp > 5000) {
    ...
    for (int i = 0; i < maxkuipnr; i++)
      currbounds[i] =
          (cppApprox(kuipdiffbounds_x, kuipdiffbounds[i], tmp_nsamp)[0]);
  }
  ...
  while (stillRepeating) {
    ...
    fts = tautString(fdist_y, x, lower, upper, (upper[0]), lower[(nsamp - 1)],
                     nsamp, extrema_mean);
    x_string = fts.string;
    anyJumpDetected = false;
    for (auto i = 0; i < (nsamp - 2); i++) {
      if (x_string[(i + 1)] != x_string[i]) {
        // capture the jump points on the first past, to remove double-checking
        // later
        if (first_jump == -1) {
          first_jump = i;
        }
    ...
    // update kuiper bounds and check points where they exceed the current
    // kuiper bounds
    ...
    if (first_kuiper_ind != -1) {
      ...
      /*this appears to be spurious logic from pmden. since kni is set to zero
       *every iteration of the while loop, the diff checks can never be true.
       *so, they are omitted from the port. we include the R logic for future
       *reference. ind1 <- diff(kni, lag = 1) > 0.04/(nsamp^2) if (sum(ind1) >
       *0) kni[c(FALSE, ind1) | c(ind1, FALSE)] <- 1 if (nsamp >= 3) { ind2 <-
       *diff(kni, lag = 2) > 0.25/(nsamp^1.5) if (sum(ind2) > 0) kni[c(FALSE,
       *FALSE, ind2) | c(FALSE, ind2, FALSE) | c(ind2, FALSE, FALSE)] <- 1
       * }
       */
